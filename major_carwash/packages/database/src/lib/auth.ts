...
export const initAuth = (orm: MikroORM) => {
  // Use a central root manager context specifically for Better Auth initialization validation
  const baseEm = orm.em.fork();

  // Helper utility mapping entities to custom BaseRepository instances
  ...
  const customAdapter: DBAdapter = {
    ...
    async create({ model, data }) {
      const repo = getRepo(baseEm, model as SupportModels);
    ...
    async transaction(fn) {
      ...
      return await baseEm.transactional(async (transactionEm) => {
        const txAdapter: any = {
          ...customAdapter,
  ...
  try {
    ...
    console.error(error);
    console.error("=================================================");
    throw error; // Re-throw so NestJS continues its standard error shutdown sequence
  }
};
