...
// 1. Import your validation function
import { Account } from "../entities/account.entity.js";
import { Session } from "../entities/session.entity.js";
import { User } from "../entities/user.entity.js";
import { Verification } from "../entities/verification.entity.js";
...
// 2. Add 'async' keyword to your initialization function
export const initAuth = (orm: MikroORM): BetterAuthServerInstance => {
  try {
    // BetterAuth initializes synchronously out of the box. 
    // Do not call any asynchronous functions (like validateDashConfig) in this block.
    const authObject = betterAuth({
      secret: process.env.BETTER_AUTH_SECRET || "fallback-secret-string-value",
      baseURL: process.env.BETTER_AUTH_URL || "http://localhost:3002/api/auth",
      database: mikroOrmAdapter(orm),
      ...
      plugins: [
        ...
        bearer(),
        {
          ...
          init: (ctx) => {
            const registry = (orm as any).getMetadata();
            // This sync mapping code remains fine
            return {};
          }
        }
      ]
    ...
    // Returns the raw structure directly without a Promise wrapper
    return {
      handler: authObject.handler,
      api: authObject.api,
    };
